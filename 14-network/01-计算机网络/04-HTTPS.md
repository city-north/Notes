...
通常，在安全性要求比较高的网站(例如银行网站)上会看到HTTPS，它本质上也是HTTP协议，只是在HTTP增加了一个SSL或TLS协议层。

TCP协议上加一层SSL或TLS协议，就构成HTTPS协议了。

SSL/TLS协议提供了加解密的机制，所以它比HTTP明文传输更安全。从图中可以看出，HTTP可以直接进入TCP传输层，也可以在TCP层上加一层SSL/TLS层，这样就先经过SSL/TLS再进入TCP传输层。这两种方式便是HTTP与HTTPS。一般HTTP的端口号为80，而HTTPS的端口号为443。
...
![image-20201025225800781](../../assets/image-20201025225800781.png)



1. 发起链接 (TCP三次握手)
2. 服务端确定此次通讯使用的加密算法与散列算法,发送证书、服务器公钥、颁发机构、有效期等到客户端
3. 客户端用通过浏览器内置的根证书验证CA证书的合法性
4. 客户端生成随机的对称秘钥,用公钥进行加密,使用散列函数计算握手的信息传给服务端
5. 服务端用私钥解密,并用散列函数验证,双方都有了此次通讯的秘钥
6. 服务端再次使用秘钥加密一段握手信息
7. 客户端浏览器用秘钥解密.并用散列算法验证,确定算法与秘钥
8. 双方根据写上好的秘钥进行通讯
...
<img src="../../assets/image-20200622210224715.png" alt="image-20200622210224715" style="zoom:50%;" />
