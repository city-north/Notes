...
![image-20210124005703899](../../../../../assets/image-20210124005703899.png)
...
![image-20210124005733428](../../../../../assets/image-20210124005733428.png)

## 课后资料

- 页面的 "入链接" 和 "出链接" 也是标记网页的重要性关键指标, 最著名的就是 Google发明的PageRank
- 300 Mutiple Choices 也是一个特殊的重定向状态码, 它会返回一个有多个连接选项的页面, 由用户自行选择要跳转的连接,用的较少
- 重定向报文里还可以用Refresh字段, 实现延时重定向, 例如 "Refresh:5; url =xxx" 告诉浏览器5妙后再刷新
- 与跳转有关的还有一个"Referer" 和"Referrer-Policy" (注意前者是一个拼写错误, 但是后者将错就错), 表示浏览器跳转的来源(即引用地址), 可用于统计分析和防盗链
- 301/302 重定向是由浏览器执行的, 对于服务器来说可以成为 "外部重定向" , 相应的,也就有服务器的"内部重定向", 直接在服务器内部跳转URL, 因为不会发出HTTP请求, 所以没有额外的性能损失